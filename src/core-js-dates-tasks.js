...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const timePerDay = 1000 * 60 ** 2 * 24;
  const difference = Math.abs(end - start);
  return Math.ceil(difference / timePerDay) + 1;
}
...
function getWeekNumberByDate(date) {
  const janFirst = new Date(date.getFullYear(), 0);
  const daysNextMon = (8 - (janFirst.getDay() || 7)) % 7;
  const nextMon = new Date(date.getFullYear(), 0, daysNextMon);
  let weekNum = daysNextMon ? 1 : 0;

  while (nextMon.getTime() < date.getTime()) {
    weekNum += 1;
...
function getNextFridayThe13th(date) {
  const nextFri13 = new Date(date.getTime());
  while (!(nextFri13.getDay() === 5 && nextFri13.getDate() === 13)) {
    nextFri13.setDate(nextFri13.getDate() + 1);
  }
  return nextFri13;
}
